FROM microsoft/dotnet:2.0-runtime-deps AS mpgpwebsocketserver

COPY --from=docker_mpgpbackendbuilder mpgpbackendbuilder/src/Mpgp.WebSocketServer/bin/release/netcoreapp2.0/publish mpgpwebsocketserver
...
WORKDIR mpgpwebsocketserver
ENTRYPOINT ["dotnet", "Mpgp.WebSocketServer.dll"]